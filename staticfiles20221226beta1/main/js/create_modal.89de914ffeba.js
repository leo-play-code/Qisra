...
const project_class = document.querySelectorAll('.project-css')
...
get_issue_mode.addEventListener('change', function handleChange(event) {
    ...
    if (issue_choose_innertext == 'Project') {
        testplan_class.forEach(element => element.classList.add('hidden'));
        project_class.forEach(element => element.classList.remove('hidden'));
    } else {
        testplan_class.forEach(element => element.classList.remove('hidden'));
        project_class.forEach(element => element.classList.add('hidden'));
    }
})
...
function send_save() {
    ...
    console.log('issue_choose_innertext',issue_choose_innertext)
    if (issue_choose_innertext == 'Project') {
        ...
        data_dict['client'] = $('#issue_client').val()
        ...
        if (issue_choose_innertext == 'Testplan'){
            target_url = '/testplan/CreateTestplan/'
            target_location = '/testplan_view/'
        }else{
            target_url = '/testplan/Create_Testplan_group/'
            target_location ='/testplan_group_view/'
        }
        if (bool_ajax == true) {
            save_btn.classList.add('hidden')
            loading_create_testplan_project.classList.remove('hidden')
            $.ajax({
                ...
                success: function (data) {
                    console.log("success");
                    window.location = '/testplan/' + data['id'] + target_location;
                },
                error: function (data) {
                    console.log("error")
                    alert("Project name has already existed ")
...
// 添加 Client
function _Open_create_client(dom){
    if (dom.innerHTML=='<img src="/static/images/journal-plus.svg">'){
        dom.innerHTML = `<button class="btn btn-warning btn-sm">cancel</button>`
    }else{
        dom.innerHTML = `<img src="/static/images/journal-plus.svg">`
    }
    create_client_css = document.querySelectorAll('.create-client-css')
    create_client_css.forEach(css_element=>{
        if (css_element.classList.contains('hidden')){
            css_element.classList.remove('hidden')
        }else{
            css_element.classList.add('hidden')
...
function _Create_Modal_Client_Tag(dom){
    create_client_input = document.getElementById('create_client_input').value
    var csrftoken = getCookie('csrftoken');
    data_dict = {}
    data_dict['csrfmiddlewaretoken'] = csrftoken;
    data_dict['add_client'] = 'add_client';
    data_dict['client_name'] = create_client_input;
    if (create_client_input.replaceAll(' ','')==''){
        createToast(false, '客戶名稱 不可為空白')
    }else{
        $.ajax({
            type: 'GET',
            ...
            success: function (data) {
                console.log('success')
                if(data['status']=='pass'){
                    finish_add_client(data)
                    createToast(true, '客戶名稱 :'+data['name']+'已成功建立!')
                }else{
                    createToast(false, '客戶名稱 :'+data['name']+'已被命名了!')
                }
                document.getElementById('create_client_input').value = ''
            },
            error: function (data) {
                console.log("error");
            }
...
function finish_add_client(data){
    new_client_name = data['name']
    new_client_id = data['id']
    // reset selectpicker
    $("#issue_client").append('<option value="'+new_client_name+'">'+new_client_name+'</option>');
    $("#issue_client").selectpicker('val', new_client_name);
    $("#issue_client").selectpicker("refresh");
}