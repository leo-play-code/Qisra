...
def Testplanview(request,pk):
    ...
    for item in testrun:
        ...
        testrun_teststep_list = TeststepforTestplan.objects.filter(testcase = item)
        ...
        testrun_dict[item] = temp_value_list
    ...
    if 'save_info' in request.POST:
        ...
        for item in add_testcase_list:
            ...
            獲取要複製的資料
            '''
            orignal_testcase = TestCasetest.objects.get(id=int(item))
            orignal_teststep = Teststep.objects.filter(testcase = orignal_testcase)
            orignal_testcase_tag = orignal_testcase.tag.all()
            '''
            建立clone檔案
    ...
    context = {'testplan':testplan,'tag_list':tag_list,'testrun_dict':testrun_dict,'count':count,'testrun_list':testrun_list,
               }
...
def TestPlanView_test(request,pk):
    ...
    return render(request,'testplan/testplanview.html',context)
