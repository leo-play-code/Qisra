...
def Testplannew_test(request):
    TestplanForm = Testplan_test_form()
    '''
    Testcase list 
    '''
    testcase_html= []
    ...
    if request.POST:
        ...
        if 'save' in request.POST:
            ...
            try:
                testplan = TestPlan_test.objects.get(name = testplan_name)
                detect_exist_name = True
            except:
                testplan = TestPlan_test.objects.create(name=testplan_name,assign=assign,stage=stage,start_date=start_date,end_date=end_date,text=text,creator=current_user.username)
                testplan.save()
                testplan = TestPlan_test.objects.get(name = testplan_name)
                print(selected_testcase_list,len(selected_testcase_list))
    ...
    return render(request,'testplans/create_testplan.html',context)

def Testplanview(request):
    testplan = TestPlan_test.objects
    context = {'testplan':}
