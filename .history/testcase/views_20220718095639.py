...
def import_excel(request):
    ...
    count_header= 0
    testcase_name= ''
    if allow_user == current_user.username:
        allow_bool = True
    if request.method == 'POST':
        ...
        讀取excel 並輸出所有sheet
        '''
        if 'delete_all_tag' in request.POST:
            TagNewTestcase.objects.all().delete()
            TestCasetest.objects.all().delete()
            Teststep.objects.all().delete()
            TestPlan_test.objects.all().delete()
            TeststepforTestplan.objects.all().delete()
            TestCaseforTestplan.objects.all().delete()
            save_excel.objects.all().delete()
        if 'check_excel' in request.POST:
            print('save btn')
            ...
            if form.is_valid():
                ...
                '''
                filename 過濾掉路徑
                /Users/leo/Desktop/tempFolder_for_KIWI/orignal/DQA-MNT-1102-ASUS OSD Message Check V1.4.xlsx
                '''
                ...
                try:
                    testcase_name_object = TestCasetest.objects.get(name=testcase_name)
                    newdoc.delete()
                    messages.info(request,'testcase has already be imported , testcase name = ' +testcase_name)
                    return redirect('import_excel')
                except:
                    testcase = TestCasetest.objects.create(name=testcase_name)
        elif 'get_sheetname' in request.POST:
            print('get_sheet_name')
            file_path = request.POST['file_path']
            testcase_name = request.POST['testcase_name']
            '''
            設定標準的list
            '''
            title_list_title = [['編號','number'],['驗 證 項 目','description'],['驗 證 條 件','condition'],['備註','remark'],['編修歷史','modify_history']]
            sheet_name = request.POST['get_sheetname']
            title_list = [] 
            ...
            '''
            print('df=',df,'title_list',title_list)
            count = 0 
            ...
            選取的標題名稱
            '''
            number_title = eval(request.POST['number'])
            description_title = eval(request.POST['description'])
            condition_title = eval(request.POST['condition'])
            remark_title = eval(request.POST['remark'])
            ...
            while i<len(df):
                ...
                if str(temp_data[0])!='nan': 
                    ...
                    if str(temp_data[3]) != 'nan':
                        ...
                        remark = ''
                    if str(temp_data[4]) != 'nan':
                        modify_history = temp_data[4]
                    else:
                        modify_history = ''
                    # data_list[int(number)]=[i,description,condition,remark,modify_history]
                    data_list[count]=[i,description,condition,remark,modify_history]
                    count+=1
                i+=1
            ...
            for item in image_list:
                ...
                for data in data_list:
                    if int(data_list[data][0])>number:
                        ...
                        break
            '''
            存取到database testcasetest
            '''
            for item in data_list:
                Teststep.objects.create(testcase=testcase_name_object,
                                            number=str(item),
                                            description=data_list[item][1],
                                            condition=data_list[item][2],
                                            remark=data_list[item][3],
                                            modify_history=data_list[item][4],)
            status = 'save_excel'
        
    print('final_status=',status)                  
    context={'status':status,'form':form,'allow_bool':allow_bool,'file_path':file_path,
             'sheet_list':sheet_list,'testcase_name':testcase_name,
             'sheetname':sheet_name,'title_list':title_list,'title_list_title':title_list_title,'count_header':count_header}
    return render(request, 'testcase/import_excel.html', context)

'''
輸出test case 所有的 excel , search testcase
'''

def TestCaseSearchView_template(request):
    ...
    if group_len != None:
        ...
        while i<group_len:
            ...
            i+=1
    if request.method == 'POST':
        testcase_id_list = request.POST.get('delete_testcase')
        testcase_id_list = ast.literal_eval(testcase_id_list)
        print('request.POST',testcase_id_list)
        for item in testcase_id_list:
            TestCasetest.objects.get(id=item).delete()
    '''
    data : 要顯示的資料
    tag_list : 已經選擇的tag
    Tag_object_list : 將已經選擇的tag object 並和到list裡面
    '''
    form = SearchCaseFormTest(request.GET)
    ...
    context =  {"form": form,'data':data,'Tag_object':Tag_object,
            'tag_list_text':tag_list_text,'testcase':testcase,'testcase_html':testcase_html,
            'edit_clone_btn_cansee':edit_clone_btn_cansee}
    return render(request,"testcase/search_new_testcase.html",context)
