...
function save_teststep_todjango(){
    ...
    teststep_tr_list_sample.forEach(element => {
        ...
        if(description_data.replace('<br />','').replace('<p>&nbsp;</p>','')!=''){
            ...
            data_dict['id_teststep_set-'+(num_count).toString()+'-condition'] = condition_data.replace('<p>&nbsp;</p>','')
            data_dict['id_teststep_set-'+(num_count).toString()+'-remark'] = remark_data.replace('<p>&nbsp;</p>','')
        }
        num_count+=1
    });
...
var teststepview_table = document.q('teststepview_table')
