...
function _Add_exists_teststep(num,data_list){
    ...
    temp_condition = data_list[2].replace(/\n/g,'</br>')
    temp_remark = data_list[3].replace(/\n/g,'</br>')
...
function _Save_all(){
    ...
    data_dict['csrfmiddlewaretoken'] = csrftoken;
    var teststep_length = document.getElementsByClassName('gradient_teststep_list').length
    for(i=1;i<=teststep_length;i++){
        var description_data = document.getElementById('id_teststep_set-'+(i-1).toString()+'-description').value

        var condition_data = CKEDITOR.instances['id_teststep_set-'+(i-1).toString()+'-condition'].getData()
        var remark_data = CKEDITOR.instances['id_teststep_set-'+(i-1).toString()+'-remark'].getData()
        data_dict['id_teststep_set-'+(i-1).toString()+'-description'] = description_data
        data_dict['id_teststep_set-'+(i-1).toString()+'-condition'] = condition_data
        data_dict['id_teststep_set-'+(i-1).toString()+'-remark'] = remark_data
    }
    data_dict['name'] = testcasename
    data_dict['tag_list[]'] = tag_list
    data_dict['save_testcase'] = 'save_testcase'
    data_dict['content'] = CKEDITOR.instances['content'].getData() 
    $.ajax({
        type: 'POST',
        url: '',
        data: data_dict,
