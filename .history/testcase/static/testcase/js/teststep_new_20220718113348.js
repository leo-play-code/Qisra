...
function SaveMode(){
    ...
    teststep_tr_list_sample.forEach(element => {
        ...
        if(description_data.replace('<br />','').replace('<p>&nbsp;</p>','')!=''){
            ...
            `<tr>
                <td>${num_count+1}</td>
                <td>${description_data.replace(replace(/(\r\n|\n|\r)/gm,"")}</td>
                <td><p></p>${condition_data.replace(/(\r\n|\n|\r)/gm,"")}</td>
                <td><p></p>${remark_data.replace(/(\r\n|\n|\r)/gm,"")}</td>
            </tr>
            `
        }
        num_count+=1
...
function save_teststep_todjango(){
    ...
    teststep_tr_list_sample.forEach(element => {
        ...
        if(description_data.replace('<br />','').replace('<p>&nbsp;</p>','')!=''){
            ...
            data_dict['id_teststep_set-'+(num_count).toString()+'-condition'] = condition_data.replace(/(\r\n|\n|\r)/gm,"")
            data_dict['id_teststep_set-'+(num_count).toString()+'-remark'] = remark_data.replace(/(\r\n|\n|\r)/gm,"")
