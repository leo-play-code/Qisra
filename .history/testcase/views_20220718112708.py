...
def debug_excel(name,data):
    ...
    remark = data[3]
    try:
        with open(python_path+"/res/"+name+".csv",'a') as fd:
            writer = csv.writer(fd)
            writer.writerow([number,description,condition,remark])
    except:
        with open(python_path+"/res/"+name+".csv",'a') as fd:
            writer = csv.writer(fd)
            writer.writerow(['number','des','cond','remark'])
    ...
    if 'save_teststep' in request.POST:
        ...
        for item in teststep_dict:
            if teststep_dict[item][0] != '':
                debug_excel('before_save',[count+1,teststep_dict[item][0],teststep_dict[item][1],teststep_dict[item][2]])
                Teststep.objects.create(testcase=testcase,
                                        number=str(count+1),
                                        description=teststep_dict[item][0],
                                        condition=teststep_dict[item][1].replace('<p>&nbsp;</p>','').replace('<br />','').replace('</br></br>','</br>'),
                                        remark=teststep_dict[item][2].replace('<p>&nbsp;</p>','').replace('<br/>','').replace('</br></br>','</br>'),)   
                debug_excel('after_save',[count+1,teststep_dict[item][0],teststep_dict[item][1].replace('<p>&nbsp;</p>',''),teststep_dict[item][2].replace('<p>&nbsp;</p>','')])
                count+=1
        tag_object_list = testcase.tag.all()
